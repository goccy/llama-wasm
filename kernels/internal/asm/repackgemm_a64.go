...
func a64RepackGemmKernel(sym string, feature string, wide bool) string {
	...
	// One body per feature level: the i8mm body takes the SMMLA nest,
	// the dotprod body falls through to the SDOT nest (the other nest
	// stays in the text unreferenced).
	if feature == "i8mm" {
		w("\tB\tgemmm")
	} else {
		w("\tB\tgemmsdot")
