...
func x64VecDotF16Kernel(sym string, _ *ConstPool, wide bool) string {
	var sb strings.Builder
	w := func(format string, args ...any) { fmt.Fprintf(&sb, format+"\n", args...) }
	args, _ := vecDotF16Args(wide)
	movPtr := "MOVL"
	if wide {
		movPtr = "MOVQ"
	}
...
func x64VecMadF16F32Kernel(sym string, _ *ConstPool, wide bool) string {
	var sb strings.Builder
	w := func(format string, args ...any) { fmt.Fprintf(&sb, format+"\n", args...) }
	args, _ := vecMadF16F32Args(wide)
	movPtr := "MOVL"
	if wide {
		movPtr = "MOVQ"
	}
