...
func runQuantMat(t *testing.T, kernel quantKernel, k int, seed uint32, zeroRow int, scale float32) {
	...
	refCheck(t, kernel, before, mem, []refArg{rPtr(int64(xOff)), rPtr(int64(yOff)), rI64(int64(k))},
		nil, []refOut{outBytes(yOff, k/256*1168)}, 0)
	want := reference(x, k)
	for i := range want {
...
func TestA64QuantizeMatQ8K4x8KernelGate(t *testing.T) {
	...
	dir := t.TempDir()
	writeRunTree(t, dir, "quantmatrun", "arm64", asm, quantMatRunSrc+quantMatDecls, quantMatRunTest, "QuantKernel", "dbg_quantize_mat_q8_K_4x8")
...
func TestX64QuantizeMatQ8K4x8KernelGate(t *testing.T) {
	...
	dir := t.TempDir()
	writeRunTree(t, dir, "quantmatrun", "amd64", asm, quantMatRunSrc+quantMatDecls, quantMatRunTest, "QuantKernel", "dbg_quantize_mat_q8_K_4x8")
