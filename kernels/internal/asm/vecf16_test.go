...
func TestVecF16KernelShape(t *testing.T) {
	a := a64VecDotF16Kernel("Fn16dotprod", nil, true)
	for _, want := range []string{"fcvtl2 v17.4s, v4.8h", "fmla v3.4s, v19.4s, v23.4s", "fmadd s24, s4, s6, s24", "faddp s0, v0.2s", "l5+48(FP)"} {
		if !strings.Contains(a, want) {
			t.Errorf("a64 vec_dot_f16 missing %q", want)
		}
	}
	if s := a64VecMadF16F32Kernel("Fn17dotprod", nil, true); !strings.Contains(s, "fmla v23.4s, v19.4s, v8.s[0]") || !strings.Contains(s, "l3+32(FP), F8") {
		t.Error("a64 vec_mad_f16_f32 shape")
	}
	x := x64VecDotF16Kernel("Fn16avx2", nil, true)
	for _, want := range []string{"VCVTPH2PS\t48(DX), Y11", "VFMADD231PS\tY11, Y7, Y3", "VFMADD231SS\tX8, X4, X13", "VHADDPS", "l5+48(FP)"} {
		if !strings.Contains(x, want) {
			t.Errorf("x64 vec_dot_f16 missing %q", want)
		}
	}
	if s := x64VecMadF16F32Kernel("Fn17avx2", nil, true); !strings.Contains(s, "VBROADCASTSS\tl3+32(FP), Y8") {
...
func TestA64VecF16KernelGate(t *testing.T) {
	_, dotBytes := vecDotF16Args(true)
	_, madBytes := vecMadF16F32Args(true)
