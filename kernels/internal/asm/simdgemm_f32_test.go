...
func TestSimdGemmF32KernelShape(t *testing.T) {
	a := a64SimdGemmF32Kernel("Fn12dotprod", true)
	...
	if n := a64SimdGemmF32Kernel("Fn12dotprod", false); !strings.Contains(n, "MOVWU\tl0+8(FP)") {
		t.Error("a64 narrow kernel: wrong argument frame")
	}
	x := x64SimdGemmF32Kernel("Fn12avx2", true)
	for _, want := range []string{
...
// every host and executes the comparison on arm64 hosts.
func TestA64SimdGemmF32KernelGate(t *testing.T) {
	_, argBytes := simdGemmF32Args(true)
	kernel := wrap("arm64", "GemmKernel", 16, argBytes, "neon", a64SimdGemmF32Kernel("GemmKernel", true))
	dir := t.TempDir()
	writeRunTree(t, dir, "gemmrun", "arm64", kernel, simdGemmRunSrc+
...
// every host and executes the comparison when the host has AVX2.
func TestX64SimdGemmF32KernelGate(t *testing.T) {
	_, argBytes := simdGemmF32Args(true)
	kernel := wrap("amd64", "GemmKernel", 16, argBytes, "avx2", x64SimdGemmF32Kernel("GemmKernel", true))
	dir := t.TempDir()
	writeRunTree(t, dir, "gemmrun", "amd64", kernel, simdGemmRunSrc+
		"\nfunc GemmKernel(m *mockModule, l0, l1, l2 int64, l3, l4, l5 int32)\nfunc trapstub()\n\nvar _ = trapstub\n",
		`package gemmrun
