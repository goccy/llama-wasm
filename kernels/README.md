...
| `dbg_gemv_mxfp4_8x8`, `dbg_gemm_mxfp4_8x8` | MXFP4 8x8 repack GEMV/GEMM (block_mxfp4x8: E8M0 column scales, fp4 nibbles through kvalues_fp4) | arm64 dotprod / i8mm, amd64 avx2 | the iq4_nl x8 bodies with the fp4 table and the E8M0 decode (2^(e-128), denormal below e = 2) as the column scales |
| `dbg_vec_dot_mxfp4_q8_0` | mxfp4 x q8_0 dot (MXFP4 rows the backend does not repack) | arm64 dotprod, amd64 avx2 | the iq4_nl dot with the fp4 table and an E8M0 scale |
| `dbg_vec_dot_iq4_xs_q8_K` | iq4_xs x q8_K dot (IQ4_XS rows: 256-quant super-blocks, 6-bit sub-block scales) | arm64 dotprod, amd64 avx2 | kvalues lookup + SDOT/VPMADDUBSW per sub-block, the 6-bit scales as i32 lane multiplies |
| `dbg_vec_dot_iq3_xxs_q8_K`, `dbg_vec_dot_iq3_s_q8_K` | iq3_xxs / iq3_s x q8_K dots (3-bit codebooks: u32 grid entries, 7-bit sign codes or sign bytes, integer sub-block scales) | arm64 dotprod, amd64 avx2 | grid words gathered through general registers, signs as +-1 bytes (keven table / TBL+CMTST), SDOT per sub-block, scales as i32 lane multiplies |
| `dbg_vec_dot_iq2_xxs_q8_K`, `dbg_vec_dot_iq2_xs_q8_K`, `dbg_vec_dot_iq2_s_q8_K` | iq2 family x q8_K dots (2-bit codebooks: u64 grid entries by 8/9/10-bit index, one or two scales per sub-block) | arm64 dotprod, amd64 avx2 | u64 gathers, +-1 sign bytes, SDOT per 16 quants |
| `dbg_vec_dot_iq1_s_q8_K`, `dbg_vec_dot_iq1_m_q8_K` | iq1 family x q8_K dots (1-bit codebook: ternary u64 grid entries, +-1 delta per group / sub-block, 3-bit scales) | arm64 dotprod, amd64 avx2 | u64 gathers, the delta as bsums (iq1_s) or an SDOT against +-1 bytes (iq1_m), two i32 totals combined as sumi + 0.125 * delta |
| `dbg_vec_dot_tq1_0_q8_K`, `dbg_vec_dot_tq2_0_q8_K` | ternary x q8_K dots (base-3 digits five per byte / 2-bit fields) | arm64 dotprod, amd64 avx2 | digits as byte multiply + two threshold compares, planes shifted out, SDOT |
| `dbg_vec_dot_q1_0_q8_0`, `dbg_vec_dot_q2_0_q8_0` | binary / 2-bit x q8_0 dots (four / two activation blocks per weight block, each with its own scale) | arm64 dotprod, amd64 avx2 | sign bits or 2-bit fields spread to bytes (TBL/CMTST, TBL/USHL), SDOT per activation block |
| `dbg_vec_dot_nvfp4_q8_0` | nvfp4 x q8_0 dot (fp4 nibbles, UE4M3 scale per 16 quants) | arm64 dotprod, amd64 avx2 | kvalues TBL, UE4M3 through an f32 table, SDOT per sub-block |
